from basics_pkg import define_Hs, np2qt, simple_plot, spectral_norm, l2_norm
...
final_hamiltonian = 'kSAT Dickson' # 'Grover' # 'kSAT' # 'molecular' # 'spin network' # 'kSAT Dickson' # 'simple Ising Dickson' # 
initial_hamiltonian = 'bit flip noisy'# PLUS ancilla' #'transverse field' #'all spins up' # 'staggered' # 'disrespect bit structure' # 'bit structure' # 
...
catalyst_interaction = 'None'#'Sigmas'#'Sigmas' # 'Sigmas' # 'bit flip noise'
catalyst1 = 'None' # 'right tilted' # 'parabola' # 'None
catalyst_interaction1 = 'None' # 'Dickson'
...
#############
np.random.seed(1234)
rising = 15#15#936#15#9936    ### rising = 15 + W=0 + h=1 is very hard (for N=5) ; rising = 2 + W=0 + h= 0.1 + N=5 is also very hard
rsnX = seeds_isingX = np.random.randint(1, 1000000, 100)[49]#15#936#67
##############
H_s, H_i, H_f, H_c = define_Hs(N_qubits, final_hamiltonian, initial_hamiltonian, annealing_schedule, catalyst, coord_catalyst = 1, 
    catalyst_interaction = catalyst_interaction, catalyst1 = catalyst1, catalyst_interaction1 = catalyst_interaction1, rotate=rot, h_mean = 1, 
    W = 0, mxdf = mxdf, number_of_ancillas = n_a, break_degeneracy = b_deg, return_HiHf = True)
######################################################################## 
######################################################################## 

print('magnitudes of Hi and Hf', l2_norm(H_i), l2_norm(H_f))
print('commutator', l2_norm( np.dot(H_f, H_i) - np.dot(H_i, H_f) ))

T_anneal = 80000#100 ## usual values to try out: T=100, dt = 0.01
dt = 10
...
for i in range(n_points):
    ...
    # curr_mod_Hs = l2_norm(H)
    # mod_Hs += [curr_mod_Hs]
    # rate_of_change_H += [(l2_norm(H - prev_Hs))/ds]
    # prev_Hs = H
...
for i in range(dims_matrices):
    ...
    yaxis = energies[:,i]
    fig, ax = simple_plot(fig, ax, xaxis, yaxis, xlabel, ylabel, markr = '-', alpha = 0.4, reduce = True)
yaxis = np.real(mean_E)
fig, ax = simple_plot(fig, ax, xaxis, yaxis, xlabel, ylabel, markr = 'kx', alpha = 0.6, reduce = True)
plt.show()

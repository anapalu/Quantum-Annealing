...
number_of_kSATs = 1000

rand_pool = rand(number_of_kSATs)
seeds = [Int(floor(1000 * x)) for x in rand_pool]

f = open("./kSAT/instances/UNIQUE$(k)SAT_N$(n)_seed$(seed).txt", "w")
write(f, "###$(number_of_kSATs) UNIQUE $(k)-SAT instances for $(n) spins\n")
for i in 1:number_of_kSATs
    inst, sol = kSAT(n, k, seeds[i])
    num_instances, _ = size(inst)
